...
namespace SocietyProApi.Domain.Diversos
{
    public static class Diverso
    {
        ...
        public static string GetBuildToken(string user)
        {
        ...
        public static bool SaveCoverPicture(string data, string destino, string nome)
        {
            var config = new ConfigurationBuilder()
                .SetBasePath(Directory.GetCurrentDirectory())
                .AddJsonFile("appsettings.json")
            ...
            var base64array = Convert.FromBase64String(data.Split(',')[1]);

            var filePath = Path.Combine(config.GetSection(key: "CurrentDirectory")[destino] + nome);
